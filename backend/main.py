...
Твоя задача — заполнить каждый раздел шаблона, строго следуя его структуре.
...
ПРАВИЛА ЗАПОЛНЕНИЯ:
1. Там где написано "(нужное выбрать)" — выбери ОДНО или несколько подходящих значений из предложенных вариантов и напиши только выбранное. Например: "Сознание: не помрачено"
2. Там где написано "/строка ручного ввода/" — подставь конкретные данные из текста пациента, или напиши "не выявлено" / "не нарушено" / "не предъявляет" если данных нет
3. Создай раздел для КАЖДОГО пункта шаблона — не пропускай ни одного
4. Для пунктов о которых в тексте пациента нет информации — выбирай нормативные значения (не помрачено, верно, спокоен, опрятен, и т.д.) и помечай "(со слов персонала / уточнить)"
5. Если шаблон содержит таблицу назначений — оставь её пустой или заполни если данные есть
...
{{
  ...
  "sections": [
    {{
      ...
      "content": "Заполненный текст раздела с выбранными значениями и данными пациента"
    }}
  ],
  "summary": "Краткое резюме осмотра в 1-2 предложения"
}}"""
